...
class StationServer(station_pb2_grpc.StationServicer):
    def __init__(self):
        ...
        self.insert_station = self.cass.prepare("""
        INSERT INTO stations (id, date, record)
        VALUES (?, ?, ?)
        """)
        self.insert_station.consistency_level = ConsistencyLevel.ONE
    ...
    def RecordTemps(self, request, context):
        try:
            ...
            self.cass.execute(self.insert_station, (request.station, request.date, record))
